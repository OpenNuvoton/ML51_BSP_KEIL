...
/**
  *  Power down wakeup each 6s for define TK power down calibration
*/
...
void main(void)
{
...
#ifdef DEMO_FREERUN
    {
        ...
        /*RTC initial for poewr down scan*/
...
TKRECHECK:
                ...
                {
                    LCDLIB_SetSymbol(SYMBOL_A_12, 0);  
                    i8LedChan = i8Ret;
                    TK_lightLED(TRUE, i8LedChan);    /* TK0 LEDG2 display OFF status */
                }

#if 0    /* for LCDView with TK demo. The power down function should not be enable. */
...
TKCHECKEND:
            ...
            /* LCD Display base on TK check result*/
            if (i8TKChan != -1)
            {
                g_u8IsPress = 1;
